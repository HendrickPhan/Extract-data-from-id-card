...
def ocr(image):
    ...
    #cv2.imshow("a",image)
    #cv2.waitKey(0)
    t = pytesseract.image_to_string(image, lang='vie',config="--psm 6 --oem 1")
    return t
...
for i in range(1,82):
    print(i)
    im_gray = cv2.imread('./test'+str(i)+'.jpg', cv2.IMREAD_GRAYSCALE)
    im_gray = cv2.GaussianBlur(im_gray,(5,5),0)
    (thresh, im_bw) = cv2.threshold(im_gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    print(thresh)
    if thresh<160:
        thresh=thresh*(1-(0.2-thresh/1000))
    else:
        thresh=thresh*(1-(0.33-thresh/1000))
    #thresh=thresh-21
    print(thresh)
